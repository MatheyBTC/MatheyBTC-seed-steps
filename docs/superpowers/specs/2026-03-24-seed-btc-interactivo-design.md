...
| 7 | PBKDF2 → Root Seed | PBKDF2 → Root Seed | `PBKDF2-HMAC-SHA512(password=NFKD(mnemonic), salt=UTF8("mnemonic")‖NFKD(passphrase), iterations=2048, dkLen=64)` → 64 bytes. Note: the `"mnemonic"` prefix is a fixed ASCII string — it is NOT NFKD-normalized. Only the user passphrase is normalized. |
...
| 13 | xprv / xpub | xprv / xpub | 78-byte serialization: version(4) + depth(1) + parent_fingerprint(4) + child_index(4) + chain_code(32) + key(33) → Base58Check. For xprv: key = `0x00 ‖ 32-byte_private_key` (padded to 33 bytes). For xpub: key = compressed 33-byte pubkey (naturally 33 bytes). |
...
| 18 | Derivación Por Nivel | Per-Level Derivation | Hardened: `HMAC-SHA512(key=chain_code, data=0x00‖privkey‖index)`; Normal: `HMAC-SHA512(key=chain_code, data=compressed_pubkey(33 bytes)‖index)` — the pubkey in the normal case MUST be the compressed 33-byte form → child privkey + child chain code |
...
| 24 | Encoding | Encoding | Bech32 for BIP84 (`bc1q...`); Bech32m for BIP86 (`bc1p...`); Base58Check for BIP44 (`1xxx`) and BIP49 (`3xxx`) |
...
- Error states: WebCrypto unavailable, computation failure, passphrase normalization error (non-NFKD-safe characters)
